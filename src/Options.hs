...
parseArgs args = 
  do let (actions,_,_) = getOpt RequireOrder options args
...
options = 
  [Option "h"
          ["host"]
          (ReqArg (\arg opt -> return opt {host = arg}) "localhost")
  ...
  ,Option "t"
          ["token"]
          (ReqArg (\arg opt -> return opt {token = Just arg}) "")
  ...
  ,Option "p"
          ...
          (ReqArg (\arg opt -> 
                     do let val = readEither arg :: Either String Int
                        case val of
                          Left err -> 
                            do BS.hPutStrLn stderr $
                                 BS.pack $
                                 "Failed reading '" ++ arg ++ "' as a number"
                               exitFailure
                          Right p -> return opt {port = p})
                  "8500")
